...
/// </summary>
namespace de.netcrave.nMVC.Utilities
{
	public sealed class UtilitiesManager : Manager
	{
		public readonly Dictionary<string, string> mimeTypes = new Dictionary<string,string>();
		private static volatile UtilitiesManager instance;
		...
		public static new UtilitiesManager Instance 
		{
			get 
			{
		...
		/// <summary>
		/// Pretties the render exception.
		/// </summary>
		/// <returns>The render exception.</returns>
		/// <param name="ex">Ex.</param>
		public string PrettyRenderException(Exception ex, string indent="")
		{
			if(ex == null) return "";
			string ret = "\n" + indent + "**** Exception Summary ****\n";
			ret += indent + "Exception Type: " + ex.GetType().ToString() + "\n";
			foreach(PropertyInfo f in ex.GetType().GetProperties())
			{
				ret += indent + f.Name + ": ";
				ret += indent + ParseExceptionProperty(f, ex, indent);
			}
			return ret + PrettyRenderException(ex.InnerException, indent + "\t") + "\n" + indent + "*** End of Exception Summary. ***\n";
		}

		/// <summary>
		/// Parses the exception property.
		/// </summary>
		/// <returns>The exception property.</returns>
		/// <param name="f">F.</param>
		/// <param name="ex">Ex.</param>
		/// <param name="indent">Indent.</param>
		private string ParseExceptionProperty(PropertyInfo f, Exception ex, string indent="")
		{
			switch(f.Name)
			{
			case "StackTrace":
				return "\n" + PrettyRenderStackTrace(new StackTrace(ex, true), indent + '\t') + "\n";
			case "InnerException":
				return (ex.InnerException != null) ? "<see below> \n" : "this is the innermost exception \n";
			default:
				return (f.GetValue(ex, null) != null) ? f.GetValue(ex, null).ToString() + "\n" : "\n";

			}
		}

		/// <summary>
		/// Pretties the render stack trace.
		/// </summary>
		/// <returns>The render stack trace.</returns>
		/// <param name="stackTrace">Stack trace.</param>
		/// <param name="indent">Indent.</param>
		public string PrettyRenderStackTrace(StackTrace stackTrace, string indent="")
		{
			string ret = "\n";
			//ret += indent + "BUG can't get file name on new StackTrace(Exception)\n";
			foreach(StackFrame sf in stackTrace.GetFrames())
			{
				string filen = sf.GetFileName();
				ret += indent + "in " + sf.GetMethod().Name + ' ';
				ret += (filen != null && filen.Contains(Path.DirectorySeparatorChar)) 
					? sf.GetFileName().Split(Path.DirectorySeparatorChar).Last() 
					: sf.GetFileName();
				ret += ' ';
				ret += "line: " +  sf.GetFileLineNumber();
				//TODO CANT GET THE FILE!
		...
		/// <summary>
		/// Parses and sanitizes POST Data and returns a Dictionary<string, string>
		/// </summary>
		/// <returns>
		/// The POST data.
		/// </returns>
		...
		/// Context.
		/// </param>
		public Dictionary<string, string> GetPOSTData (HttpListenerContext ctx)
		{
			...
			if (text == null)
			{
				nMVCLogger.Instance.Warn("error parsing post data");
				return null;
			}
			...
			foreach (string param in rawParams)
			{
				...
				ret.Add (key.Trim ().ToUpper (), value);
			}
		...
		/// <summary>
		/// Parses and sanitizes the Headers and returns a Dictionary<string, string> 
		/// </summary>
		/// <returns>
		/// The headers.
		/// </returns>
		...
		/// Context.
		/// </param>
		public Dictionary<string, string> GetHeaders (HttpListenerContext ctx)
		{
			Dictionary<string, string> ret = new Dictionary<string, string> ();

			foreach (string headerKey in ctx.Request.Headers.AllKeys)
			{
				ret.Add (headerKey.Trim (), ctx.Request.Headers [headerKey]);
			}
