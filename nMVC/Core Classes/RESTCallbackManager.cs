	...
	public class RESTCallbackManager : Manager
	{
		...
		public void RegisterGlobalCallbackHandlers()
		{
			...
			foreach(Type t in services)
			{
				...
				{
					Log.WriteLine("Adding REST Service: " + t.ToString());
					if(t.GetProperty("Instance") != null)
					{
						...
						foreach(MethodInfo mi in contracts)
						{
							...
							if(mi.GetCustomAttributes(true).Any(a => a.GetType() == typeof(RESTContract)))
							{
								//Mark
								RESTContract data = (RESTContract) mi.GetCustomAttributes(true)
									.Single(a => a.GetType() == typeof(RESTContract));

								Log.WriteLine("Adding data contract for " 								
									+ "API Version/Level: "
									+ data.APILevel
									+ " Contract: " 
								...
								case RESTContract.RequestType.GET:
								...
								case RESTContract.RequestType.POST:
								...
								case RESTContract.RequestType.PUT:															
								case RESTContract.RequestType.DELETE:								
								default:
									Log.WriteLine("unsupported REST contract based on specified request type");
									break;
								}
							}
							if (mi.GetCustomAttributes(true).Any(a => a.GetType() == typeof(RESTExpectsParam)))
							{
								...
								{
									Log.WriteLine("adding data input handler for: "									
										+ " Contract: " 
										+ data.RequestHandlerName 
										+ " Expects param: "
										+ eparam.ExpectKey);
